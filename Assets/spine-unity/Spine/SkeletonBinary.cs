	...
	public class SkeletonBinary
	{
		...
		public SkeletonData ReadSkeletonData(Stream input)
		{
			...
			for (int num5 = ReadVarint(input, true); k < num5; k++)
			{
				...
				ikConstraintData.compress = ReadBoolean(input);
				ikConstraintData.stretch = ReadBoolean(input);
				ikConstraintData.uniform = ReadBoolean(input);
		...
		private void ReadAnimation(string name, Stream input, SkeletonData skeletonData)
		{
			...
			for (int num18 = ReadVarint(input, true); num17 < num18; num17++)
			{
				...
				{
					ikConstraintTimeline.SetFrame(num20, ReadFloat(input), ReadFloat(input), ReadSByte(input), ReadBoolean(input), ReadBoolean(input));
					if (num20 < num19 - 1)
					{
				...
				num = Math.Max(num, ikConstraintTimeline.frames[(num19 - 1) * 5]);
			}
			int num21 = 0;
