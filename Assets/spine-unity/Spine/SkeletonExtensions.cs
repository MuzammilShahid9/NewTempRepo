	...
	public static class SkeletonExtensions
	{
		...
		internal static void SetPropertyToSetupPose(this Skeleton skeleton, int propertyID)
		{
			...
			case TimelineType.IkConstraint:
			{
				...
				obj5.stretch = obj5.data.stretch;
				break;
			}
		...
		public static void SetSlotAttachmentsToSetupPose(this Skeleton skeleton)
		{
			...
			{
				Slot obj = items[i];
				string attachmentName = obj.data.attachmentName;
				obj.Attachment = (string.IsNullOrEmpty(attachmentName) ? null : skeleton.GetAttachment(i, attachmentName));
			}
		}
		...
		public static void PoseWithAnimation(this Skeleton skeleton, string animationName, float time, bool loop = false)
		{
			...
			if (animation != null)
			{
				animation.Apply(skeleton, 0f, time, loop, null, 1f, MixBlend.Setup, MixDirection.In);
			}
		}
