...
namespace Spine.Unity
{
	...
	public class MeshGenerator
	{
		...
		public void AddSubmesh(SubmeshInstruction instruction, bool updateTriangles = true)
		{
			...
			for (int i = instruction.startSlot; i < instruction.endSlot; i++)
			{
				...
				else
				{
					...
					num3 = worldVerticesLength >> 1;
					num4 = meshAttachment.triangles.Length;
				}
				if (pmaVertexColors)
				{
					color.a = (byte)(num2 * slot2.a * color2.a);
				...
				{
					clipper.ClipTriangles(array, num3 << 1, array3, num4, array2);
					array = clipper.clippedVertices.Items;
					num3 = clipper.clippedVertices.Count >> 1;
					array3 = clipper.clippedTriangles.Items;
					num4 = clipper.clippedTriangles.Count;
				...
				if (num3 != 0 && num4 != 0)
				{
					if (tintBlack)
					{
						AddAttachmentTintBlack(slot2.r2, slot2.g2, slot2.b2, num3);
					}
					int count = vertexBuffer.Count;
					int num5 = count + num3;
					int num6 = vertexBuffer.Items.Length;
					if (num5 > num6)
					{
						int num7 = (int)((float)num6 * 1.3f);
						if (num7 < num5)
						{
							num7 = num5;
						}
						Array.Resize(ref vertexBuffer.Items, num7);
						Array.Resize(ref uvBuffer.Items, num7);
						Array.Resize(ref colorBuffer.Items, num7);
					}
					vertexBuffer.Count = (uvBuffer.Count = (colorBuffer.Count = num5));
					...
					if (count == 0)
					{
						for (int j = 0; j < num3; j++)
						{
							...
							float num11 = array[num9 + 1];
							items2[num8].x = num10;
							items2[num8].y = num11;
							items2[num8].z = z;
							items3[num8].x = array2[num9];
							items3[num8].y = array2[num9 + 1];
							items4[num8] = color;
							if (num10 < vector.x)
							{
								vector.x = num10;
					...
					if (updateTriangles)
					{
						...
						int num16 = count2 + num4;
						if (num16 > exposedList.Items.Length)
						{
							Array.Resize(ref exposedList.Items, num16);
						}
						exposedList.Count = num16;
			...
			for (int num17 = items6.Length; m < num17; m++)
			{
				items6[m] = 0;
			}
			submeshIndex++;
		...
		public void FillVertexData(Mesh mesh)
		{
			...
			{
				float num2 = (meshBoundsMax.x - meshBoundsMin.x) * 0.5f;
				float num3 = (meshBoundsMax.y - meshBoundsMin.y) * 0.5f;
				mesh.bounds = new Bounds
				{
					center = new Vector3(meshBoundsMin.x + num2, meshBoundsMin.y + num3),
					extents = new Vector3(num2, num3, meshBoundsThickness * 0.5f)
				};
			}
