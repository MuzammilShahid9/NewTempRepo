	...
	public class SkeletonRagdoll2D : MonoBehaviour
	{
		...
		public void Apply()
		{
			...
			foreach (KeyValuePair<Bone, Transform> item in boneTable)
			{
				...
				if (key == bone3)
				{
					...
					ragdollRoot.SetParent(base.transform, false);
		...
		private void UpdateSpineSkeleton(ISkeletonAnimation animatedSkeleton)
		{
			...
			foreach (KeyValuePair<Bone, Transform> item in boneTable)
			{
				...
				Transform transform = (flag5 ? ragdollRoot : boneTable[key.Parent]);
		...
		private static List<Collider2D> AttachBoundingBoxRagdollColliders(Bone b, GameObject go, Skeleton skeleton, float gravityScale)
		{
			...
			foreach (Slot slot in skeleton.Slots)
			{
				...
				foreach (Attachment item2 in list2)
				{
					...
					{
						flag = true;
						PolygonCollider2D item = SkeletonUtility.AddBoundingBoxAsComponent(boundingBoxAttachment, slot, go, false);
						list.Add(item);
					}
				}
				if (flag)
