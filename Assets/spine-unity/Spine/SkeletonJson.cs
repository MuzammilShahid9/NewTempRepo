	...
	public class SkeletonJson
	{
		...
		private void ReadAnimation(Dictionary<string, object> map, string name, SkeletonData skeletonData)
		{
			...
			if (map.ContainsKey("deform"))
			{
				foreach (KeyValuePair<string, object> item20 in (Dictionary<string, object>)map["deform"])
				{
					...
					foreach (KeyValuePair<string, object> item21 in (Dictionary<string, object>)item20.Value)
					{
						...
						{
							List<object> obj7 = (List<object>)item22.Value;
							...
							int num17 = 0;
							foreach (Dictionary<string, object> item23 in obj7)
							{
								float[] array;
			...
			if (map.ContainsKey("drawOrder") || map.ContainsKey("draworder"))
			{
				...
				int num19 = 0;
				foreach (Dictionary<string, object> item24 in obj8)
				{
					int[] array2 = null;
			...
			if (map.ContainsKey("events"))
			{
				...
				int num26 = 0;
				foreach (Dictionary<string, object> item26 in obj9)
				{
					...
					{
						@event.volume = GetFloat(item26, "volume", eventData.Volume);
						@event.balance = GetFloat(item26, "balance", eventData.Balance);
					}
