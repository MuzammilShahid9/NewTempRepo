...
{
	public class IDRoleAnimation : MonoBehaviour
	{
		private static IDRoleAnimation instance;
		...
		public static IDRoleAnimation Instance
		{
			get
			{
				return instance;
		...
		{
			LoadRoleType();
			currentRole = IDInfoManager.Instance.GetCurrIDInfo().roleType;
		...
		public void AddRole(RoleType role)
		{
			...
			iDRoleAnim.anim = new List<string>();
			IDStepManager.Instance.currentAction.roleConfig.roles.Add(role, iDRoleAnim);
		}
		...
		{
			if (!IDStepManager.Instance.currentAction.roleConfig.roles.ContainsKey(currentRole))
			{
				AddRole(currentRole);
			}
			if (IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim == null)
			{
				IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim = new List<string>();
			}
			if (selectAnim.captionText.text == "Walk")
			{
				...
				IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.Add(text);
			}
			else if (selectAnim.captionText.text == "Run")
			{
				...
				AddText(text2);
				IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.Add(text2);
			}
			else if (selectAnim.captionText.text == "Flash")
			{
				...
				AddText(text3);
				IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.Add(text3);
			}
			else
			{
				AddText(selectAnim.captionText.text);
				IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.Add(selectAnim.options[currentAnim].text);
			}
		...
		{
			DelText();
			if (IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.Count > 0)
			{
				IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.RemoveAt(IDStepManager.Instance.currentAction.roleConfig.roles[currentRole].anim.Count - 1);
			}
			IDStepManager.Instance.UpdateActionInfo();
		}
